...
#define XNSU_SULOG_MAX_QUEUED 256U
#define XNSU_SULOG_MAX_PAYLOAD_LEN 2048U
#define XNSU_SULOG_MAX_ARG_STRINGS 0x7FFFFFFF
#define XNSU_SULOG_MAX_ARG_CHUNK 256U
#define XNSU_SULOG_MAX_FILENAME_LEN 256U
...
static struct xnsu_event_queue sulog_queue;
...
struct xnsu_sulog_identity {
    __u32 uid;
    __u32 euid;
};

static struct user_arg_ptr xnsu_sulog_user_argv(const char __user *const __user *argv_user)
{
...
static const char __user *xnsu_sulog_get_user_arg_ptr(struct user_arg_ptr argv, int nr)
{
...
static __u32 xnsu_sulog_copy_empty_string(char *dst)
{
    dst[0] = '\0';
    return 1;
}

static __u32 xnsu_sulog_copy_filename(const char __user *filename_user, char *dst, __u32 dst_len)
{
    long ret;
    ...
    if (!filename_user)
        return xnsu_sulog_copy_empty_string(dst);
...
static __u32 xnsu_sulog_flatten_argv(const char __user *const __user *argv_user, char *dst, __u32 dst_len)
{
    struct user_arg_ptr argv = xnsu_sulog_user_argv(argv_user);
    char arg[XNSU_SULOG_MAX_ARG_CHUNK];
    __u32 used = 0;
    int i;
    ...
    for (i = 0; i < XNSU_SULOG_MAX_ARG_STRINGS; i++) {
        ...
        arg_user = xnsu_sulog_get_user_arg_ptr(argv, i);
        if (!arg_user)
            break;
        if (IS_ERR(arg_user))
            return xnsu_sulog_copy_empty_string(dst);
                                                         ...
                                                         const char __user *const __user *argv_user, gfp_t gfp)
{
    ...
    payload = kzalloc(XNSU_SULOG_MAX_PAYLOAD_LEN, gfp);
    if (!payload)
        goto out_free_pending;

    event = payload;
    xnsu_sulog_fill_task_info(event, event_type, 0);

    remaining = XNSU_SULOG_MAX_PAYLOAD_LEN - sizeof(*event);
    filename_buf = (char *)payload + sizeof(*event);
    filename_len = xnsu_sulog_copy_filename(filename_user, filename_buf, min(remaining, XNSU_SULOG_MAX_FILENAME_LEN));
    if (!filename_len)
        goto out_free_payload;

    remaining -= filename_len;
    argv_buf = filename_buf + filename_len;
    argv_len = xnsu_sulog_flatten_argv(argv_user, argv_buf, remaining);
    if (!argv_len)
        goto out_free_payload;
...
out_free_pending:
    kfree(pending);
out_drop:
    xnsu_event_queue_drop(&sulog_queue);
    return NULL;
}
                                                                    ...
                                                                    gfp_t gfp)
{
    ...
    xnsu_sulog_set_identity(event, identity);
    return pending;
}

int __init xnsu_sulog_events_init(void)
{
    xnsu_event_queue_init(&sulog_queue, XNSU_SULOG_MAX_QUEUED, XNSU_SULOG_MAX_PAYLOAD_LEN);
    return 0;
}

void __exit xnsu_sulog_events_exit(void)
{
    xnsu_event_queue_destroy(&sulog_queue);
}

static void xnsu_sulog_free_pending(struct xnsu_sulog_pending_event *pending)
{
    if (!pending)
        return;
...
void xnsu_sulog_emit_pending(struct xnsu_sulog_pending_event *pending, int retval, gfp_t gfp)
{
    ...
    event = pending->payload;
    event->retval = retval;
    xnsu_event_queue_push(&sulog_queue, pending->event_type, 0, pending->payload, pending->payload_len, gfp);
    xnsu_sulog_free_pending(pending);
}

int xnsu_sulog_emit_grant_root(int retval, __u32 uid, __u32 euid, gfp_t gfp)
{
    struct xnsu_sulog_pending_event *pending;
    struct xnsu_sulog_identity identity = {
        .uid = uid,
        .euid = euid,
    };

    pending = xnsu_sulog_capture_grant_root(&identity, gfp);
    if (!pending)
        return 0;

    xnsu_sulog_emit_pending(pending, retval, gfp);
    return 0;
}
