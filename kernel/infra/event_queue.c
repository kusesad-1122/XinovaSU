...
struct xnsu_event_queue_node {
    struct list_head list;
    struct xnsu_event_record_hdr hdr;
    __u8 payload[];
};

static size_t xnsu_event_queue_record_size(__u32 payload_len)
{
    return sizeof(struct xnsu_event_record_hdr) + payload_len;
}

static void xnsu_event_queue_note_drop_locked(struct xnsu_event_queue *queue, __u64 seq)
{
    queue->dropped_total++;
...
static bool xnsu_event_queue_has_data_locked(const struct xnsu_event_queue *queue)
{
    return queue->dropped_pending || queue->dropped_inflight || !list_empty(&queue->pending);
}

static void xnsu_event_queue_mark_closed(struct xnsu_event_queue *queue)
{
...
void xnsu_event_queue_init(struct xnsu_event_queue *queue, __u32 max_queued, __u32 max_payload_len)
{
...
{
    struct xnsu_event_queue_node *node, *tmp;
    unsigned long irq_flags;

    xnsu_event_queue_mark_closed(queue);
                         ...
                         gfp_t gfp)
{
    ...
    if (!node || (queue->max_queued && queue->queued >= queue->max_queued)) {
        xnsu_event_queue_note_drop_locked(queue, seq);
        wake = true;
...
static int xnsu_event_queue_wait_ready(struct xnsu_event_queue *queue, int file_flags)
{
    int ret;

    for (;;) {
        if (xnsu_event_queue_has_data(queue)) {
            return 0;
        }
        ...
        ret = wait_event_interruptible(queue->read_wait, queue->closed || xnsu_event_queue_has_data(queue));
        if (ret) {
            return ret;
        }
    }
...
static ssize_t xnsu_event_queue_read_drop(struct xnsu_event_queue *queue, char __user *buf, size_t count)
{
    struct xnsu_event_record_hdr hdr;
    struct xnsu_event_queue_dropped_info info;
    size_t record_size = xnsu_event_queue_record_size(sizeof(info));
    unsigned long irq_flags;
    ...
    hdr.type = XNSU_EVENT_QUEUE_TYPE_DROPPED;
    hdr.flags = XNSU_EVENT_RECORD_FLAG_INTERNAL;
    hdr.len = sizeof(info);
...
static ssize_t xnsu_event_queue_read_node(struct xnsu_event_queue *queue, char __user *buf, size_t count)
{
    struct xnsu_event_queue_node *node;
    ...
    node = list_entry(first, struct xnsu_event_queue_node, list);
    record_size = xnsu_event_queue_record_size(node->hdr.len);
    if (count < record_size) {
        spin_unlock_irqrestore(&queue->lock, irq_flags);
        return -EMSGSIZE;
    }
...
ssize_t xnsu_event_queue_read(struct xnsu_event_queue *queue, char __user *buf, size_t count, int file_flags)
{
    ...
    ret = xnsu_event_queue_wait_ready(queue, file_flags);
    if (ret) {
        copied = ret;
        goto out_unlock;
    }
    ...
    while (count > 0) {
        ret = xnsu_event_queue_read_drop(queue, buf, count);
        if (ret < 0) {
            if (!copied) {
                copied = ret;
            }
        ...
        ret = xnsu_event_queue_read_node(queue, buf, count);
        if (ret < 0) {
            if (!copied) {
                copied = ret;
            }
...
__poll_t xnsu_event_queue_poll(struct xnsu_event_queue *queue, struct file *file, poll_table *wait)
{
    __poll_t mask = 0;
    unsigned long irq_flags;
    ...
    spin_lock_irqsave(&queue->lock, irq_flags);
    if (xnsu_event_queue_has_data_locked(queue)) {
...
void xnsu_event_queue_close(struct xnsu_event_queue *queue)
{
    xnsu_event_queue_mark_closed(queue);
    wake_up_interruptible_poll(&queue->read_wait, EPOLLHUP | POLLHUP);
}

bool xnsu_event_queue_has_data(struct xnsu_event_queue *queue)
{
    bool has_data;
    unsigned long irq_flags;

    spin_lock_irqsave(&queue->lock, irq_flags);
    has_data = xnsu_event_queue_has_data_locked(queue);
    spin_unlock_irqrestore(&queue->lock, irq_flags);
