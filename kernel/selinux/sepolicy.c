...
#define xnsu_hashtab_for_each(htab, cur) xnsu_hash_for_each(htab.htable, htab.size, cur)
#else
#define xnsu_hashtab_for_each(htab, cur) xnsu_hash_for_each(htab->htable, htab->size, cur)
#endif
...
#define avtab_for_each(avtab, cur) xnsu_hash_for_each(avtab.htable, avtab.nslot, cur);
                         ...
                         struct perm_datum *perm, int effect, bool invert)
{
    ...
    if (src == NULL) {
        ...
        if (strip_av(effect, invert)) {
            xnsu_hashtab_for_each(db->p_types.table, node)
            {
        ...
        struct hashtab_node *node;
        xnsu_hashtab_for_each(db->p_classes.table, node)
        {
...
#define xnsu_kvrealloc(p, new_size, _old_size) kvrealloc(p, new_size, GFP_KERNEL)
...
#define xnsu_kvrealloc(p, new_size, old_size) kvrealloc(p, old_size, new_size, GFP_KERNEL)
#else
...
void *xnsu_kvrealloc_compat(const void *p, size_t oldsize, size_t newsize, gfp_t flags)
{
    void *newp;
...
#define xnsu_kvrealloc(p, new_size, old_size) xnsu_kvrealloc_compat(p, old_size, new_size, GFP_KERNEL)
#endif
...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
    ...
    struct ebitmap *new_type_attr_map_array =
        xnsu_kvrealloc(db->type_attr_map_array, value * sizeof(struct ebitmap), (value - 1) * sizeof(struct ebitmap));
    ...
    struct type_datum **new_type_val_to_struct = xnsu_kvrealloc(
    ...
    char **new_val_to_name_types =
        xnsu_kvrealloc(db->sym_val_to_name[SYM_TYPES], sizeof(char *) * value, sizeof(char *) * (value - 1));
