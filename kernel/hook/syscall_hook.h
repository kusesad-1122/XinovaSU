#ifndef __XNSU_H_XNSU_SYSCALL_HOOK
#define __XNSU_H_XNSU_SYSCALL_HOOK
...
extern syscall_fn_t *xnsu_syscall_table;

// Dispatcher slot number in syscall table
extern int xnsu_dispatcher_nr;

// Syscall hook handler type.
// orig_nr: the original syscall number before redirection
// regs: the original pt_regs from userspace
// Handler is responsible for calling xnsu_syscall_table[orig_nr](regs) if needed.
typedef long (*xnsu_syscall_hook_fn)(int orig_nr, const struct pt_regs *regs);
...
// a real syscall entry (e.g. xnsusd hooking __NR_execve/__NR_read/__NR_fstat).
...
// xnsu_syscall_table_hook(), and remove the entry from the tracking list.
// Use this to cleanly undo a direct hook when it is no longer needed
// (e.g. xnsusd unhooking __NR_read after init.rc injection is done).
void xnsu_syscall_table_unhook(int nr);

void xnsu_syscall_hook_init(void);
void xnsu_syscall_hook_exit(void);
