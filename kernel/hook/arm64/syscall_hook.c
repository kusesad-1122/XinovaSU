...
syscall_fn_t *xnsu_syscall_table = NULL;
...
void xnsu_syscall_table_hook(int nr, syscall_fn_t fn, syscall_fn_t *old)
{
    if (xnsu_syscall_table == NULL)
        return;
    if (nr < 0 || nr >= __NR_syscalls) {
    ...
    syscall_fn_t orig = READ_ONCE(xnsu_syscall_table[nr]);
    if (old)
        *old = orig;
...
static int __init xnsu_find_ni_syscall_slots(int *out_slots, int max_slots)
{
    ...
    ni_syscall = (unsigned long)xnsu_resolve_symbol_for_functable_hook("__arm64_sys_ni_syscall");
...
{
    if (regs->syscallno != xnsu_dispatcher_nr)
        return -ENOSYS;
...
void __init xnsu_syscall_hook_init(void)
{
    ...
    xnsu_syscall_table = (syscall_fn_t *)xnsu_resolve_symbol_for_functable_hook("sys_call_table");
    ...
    xnsu_syscall_table_hook(xnsu_dispatcher_nr, (syscall_fn_t)xnsu_syscall_dispatcher, NULL);
