...
long __nocfi xnsu_hook_getdents64(int orig_nr, const struct pt_regs *regs)
{
    ...
    if (!xnsu_path_hide_should_filter_dents())
        return xnsu_syscall_table[orig_nr](regs);

    // Run the real syscall, then drop hidden entries. If a whole batch turns out
    // to be hidden, filtering yields 0 bytes -- which libc reads as end-of-dir,
    // prematurely stopping enumeration. So loop and fetch the next batch (the
    // real syscall has already advanced the directory position) until we have a
    // non-empty result or hit the true end of directory.
    do {
