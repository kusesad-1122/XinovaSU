...
#define XNSU_P4D_TO_PHYS(p4d) ((unsigned long)p4d_pfn(p4d) << PAGE_SHIFT)
#define XNSU_PUD_TO_PHYS(pud) ((unsigned long)pud_pfn(pud) << PAGE_SHIFT)
#define XNSU_PMD_TO_PHYS(pmd) ((unsigned long)pmd_pfn(pmd) << PAGE_SHIFT)
#define XNSU_PTE_TO_PHYS(pte) ((unsigned long)pte_pfn(pte) << PAGE_SHIFT)
...
unsigned long phys_from_virt(unsigned long addr, int *err)
{
    ...
    *err = 0;
    ...
    if (p4d_leaf(*p4d)) {
        ...
        return XNSU_P4D_TO_PHYS(*p4d) + ((addr & ~P4D_MASK));
    }
    ...
    if (pud_leaf(*pud)) {
        ...
        return XNSU_PUD_TO_PHYS(*pud) + ((addr & ~PUD_MASK));
    }
    ...
    if (pmd_leaf(*pmd)) {
        ...
        return XNSU_PMD_TO_PHYS(*pmd) + ((addr & ~PMD_MASK));
    }
    ...
    return XNSU_PTE_TO_PHYS(*pte) + ((addr & ~PAGE_MASK));
...
static int xnsu_patch_text_nosync(void *dst, void *src, size_t len, int flags)
{
    ...
    if (!ret) {
        if (flags & XNSU_PATCH_TEXT_FLUSH_ICACHE)
            xnsu_flush_icache((uintptr_t)dst, (uintptr_t)dst + len);
        if (flags & XNSU_PATCH_TEXT_FLUSH_DCACHE)
            xnsu_flush_dcache(dst, len);
    }
...
static int xnsu_patch_text_cb(void *arg)
{
    ...
    if (atomic_inc_return(&pp->cpu_count) == num_online_cpus()) {
        ret = xnsu_patch_text_nosync(dst, src, len, flags);
...
int xnsu_patch_text(void *dst, void *src, size_t len, int flags)
{
    ...
    return stop_machine(xnsu_patch_text_cb, &info, cpu_online_mask);
}
