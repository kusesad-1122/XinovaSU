...
sys_call_ptr_t *xnsu_syscall_table = NULL;
int xnsu_dispatcher_nr = -1;
...
static xnsu_syscall_hook_fn syscall_hooks[__NR_syscalls];
...
{
    if (xnsu_syscall_table == NULL)
        return -ENOENT;
    if (nr < 0 || nr >= __NR_syscalls)
        return -EINVAL;

    pr_info("patch syscall %d, 0x%lx -> 0x%lx\n", nr, (unsigned long)READ_ONCE(xnsu_syscall_table[nr]),
            (unsigned long)fn);

    if (xnsu_patch_text(&xnsu_syscall_table[nr], &fn, sizeof(fn), XNSU_PATCH_TEXT_FLUSH_DCACHE)) {
...
void xnsu_syscall_table_hook(int nr, sys_call_ptr_t fn, sys_call_ptr_t *old)
{
    if (xnsu_syscall_table == NULL)
        return;
    if (nr < 0 || nr >= __NR_syscalls) {
    ...
    sys_call_ptr_t orig = READ_ONCE(xnsu_syscall_table[nr]);
    if (old)
        *old = orig;
...
void xnsu_syscall_table_unhook(int nr)
{
    int i;
...
static int xnsu_find_ni_syscall_slots(int *out_slots, int max_slots)
{
    unsigned long ni_syscall;
    int i, count = 0;

    if (!xnsu_syscall_table || max_slots <= 0)
        return 0;

    ni_syscall = (unsigned long)xnsu_resolve_symbol_for_functable_hook("__x64_sys_ni_syscall");
    ...
    for (i = 0; i < __NR_syscalls && count < max_slots; i++) {
        if ((unsigned long)xnsu_syscall_table[i] == ni_syscall) {
            out_slots[count++] = i;
...
static long __nocfi xnsu_syscall_dispatcher(const struct pt_regs *regs)
{
    if (regs->orig_ax != xnsu_dispatcher_nr)
        return -ENOSYS;
    ...
    if (likely(orig_nr >= 0 && orig_nr < __NR_syscalls)) {
        xnsu_syscall_hook_fn fn = READ_ONCE(syscall_hooks[orig_nr]);
        if (likely(fn))
...
int xnsu_register_syscall_hook(int nr, xnsu_syscall_hook_fn fn)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return -EINVAL;
...
void xnsu_unregister_syscall_hook(int nr)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return;
...
bool xnsu_has_syscall_hook(int nr)
{
    if (nr < 0 || nr >= __NR_syscalls)
        return false;
...
void __init xnsu_syscall_hook_init(void)
{
    int ni_slot;

    memset(syscall_hooks, 0, sizeof(syscall_hooks));

    xnsu_syscall_table = (sys_call_ptr_t *)xnsu_resolve_symbol_for_functable_hook("sys_call_table");
    pr_info("sys_call_table=0x%lx", (unsigned long)xnsu_syscall_table);

    if (!xnsu_syscall_table)
        return;

    // Find one ni_syscall slot for the dispatcher
    if (xnsu_find_ni_syscall_slots(&ni_slot, 1) < 1) {
        pr_err("failed to find ni_syscall slot for dispatcher\n");
        return;
    }

    xnsu_dispatcher_nr = ni_slot;
    xnsu_syscall_table_hook(xnsu_dispatcher_nr, (sys_call_ptr_t)xnsu_syscall_dispatcher, NULL);
    pr_info("dispatcher installed at slot %d\n", xnsu_dispatcher_nr);
}

void __exit xnsu_syscall_hook_exit(void)
{
    int i;
    ...
    for (i = 0; i < hooked_count; i++) {
        ...
        if (xnsu_patch_text(&xnsu_syscall_table[nr], &orig, sizeof(orig), XNSU_PATCH_TEXT_FLUSH_DCACHE)) {
...
clear_state:
    ...
    // (done by xnsu_syscall_hook_manager_exit before calling us), so no new
    // dispatches will occur.
    memset(syscall_hooks, 0, sizeof(syscall_hooks));
    xnsu_dispatcher_nr = -1;
