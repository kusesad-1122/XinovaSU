#ifndef __XNSU_H_LSM_HOOK
#define __XNSU_H_LSM_HOOK
...
#define XNSU_LSM_HOOK_HEADS_TYPE struct lsm_static_calls_table
#else
#define XNSU_LSM_HOOK_HEADS_TYPE struct security_hook_heads
#endif

struct xnsu_lsm_hook {
...
// runtime lock for security_hook_heads mutations; callers rely on XinovaSU's
...
// for restoration by xnsu_lsm_unhook() or the global xnsu_lsm_hook_exit().
int xnsu_lsm_hook(struct xnsu_lsm_hook *hook);

// Restore the LSM hook entry previously patched by xnsu_lsm_hook() back to the
...
// after XinovaSU-specific logic.
int xnsu_register_lsm_hook(struct xnsu_lsm_hook *hook);

// Undo xnsu_register_lsm_hook() and restore the original BPF LSM handler.
