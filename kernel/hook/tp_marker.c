...
int xnsu_tp_marker_reg_count(void)
{
    return tracepoint_reg_count;
}

void xnsu_tp_marker_lock(unsigned long *flags)
{
    spin_lock_irqsave(&tracepoint_reg_lock, *flags);
}

void xnsu_tp_marker_unlock(unsigned long *flags)
{
    spin_unlock_irqrestore(&tracepoint_reg_lock, *flags);
}

void xnsu_tp_marker_inc_reg_count(void)
{
    tracepoint_reg_count++;
}

void xnsu_tp_marker_dec_reg_count(void)
{
    tracepoint_reg_count--;
}

void xnsu_clear_task_tracepoint_flag_if_needed(struct task_struct *t)
{
    unsigned long flags;
    spin_lock_irqsave(&tracepoint_reg_lock, flags);
    if (tracepoint_reg_count <= 1) {
        xnsu_clear_task_tracepoint_flag(t);
    }
    spin_unlock_irqrestore(&tracepoint_reg_lock, flags);
}
...
void xnsu_mark_running_process_locked(void)
{
    ...
    for_each_process_thread (p, t) {
        ...
        bool xnsu_root_process = uid == 0 && is_task_xnsu_domain(cred);
        ...
        if (xnsu_root_process || is_zygote_process || is_shell || is_init || xnsu_is_allow_uid(uid) ||
            xnsu_path_hide_is_target(uid) || xnsu_vpn_hide_is_target(uid)) {
            xnsu_set_task_tracepoint_flag(t);
...
void xnsu_mark_running_process(void)
{
    unsigned long flags;
    spin_lock_irqsave(&tracepoint_reg_lock, flags);
    if (tracepoint_reg_count <= 1) {
...
int xnsu_get_task_mark(pid_t pid)
{
...
int xnsu_set_task_mark(pid_t pid, bool mark)
{
    struct task_struct *task;
    int ret = -ESRCH;
    ...
    if (task) {
        ...
        rcu_read_unlock();
        if (mark) {
            xnsu_set_task_tracepoint_flag(task);
            pr_info("tp_marker: marked task pid=%d comm=%s\n", pid, task->comm);
        } else {
            xnsu_clear_task_tracepoint_flag(task);
