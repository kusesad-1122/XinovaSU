...
pub fn on_post_data_fs() -> Result<()> {
    ...
    // of xnsusd's normal mutation commands.
    ...
    // Re-apply persisted kernel uname spoof (if configured).
    crate::kernel_spoof::apply_from_config();

    // Re-apply persisted per-app network isolation (if configured).
    crate::net_isolate::apply_from_config();

    // Re-apply persisted per-app path hiding (if configured).
    ...
    // Apply FolkPatch-style bootloader / verified-boot property hiding if the
    // user enabled it (marker file). Only rewrites props that already exist.
    if crate::bl_hide::is_enabled()
        && let Err(e) = crate::bl_hide::run_bl_hide()
    {
        warn!("bl_hide failed: {e}");
    }
    ...
    // per-app on setuid just like module mounts. Add-only: never wipe, so any
    // module entries already present are preserved.
...
pub fn soft_reboot() -> Result<()> {
    // Check it to avoid user click "soft_reboot" in manager when version mismatch
    if let Err(e) = ksucalls::ensure_uapi_version_matched() {
        warn!("{e:#}, skip soft_reboot");
        return Ok(());
    }
