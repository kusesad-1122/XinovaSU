...
pub fn patch(args: BootPatchArgs) -> Result<()> {
    let inner = move || {
        ...
        if !no_install {
            ...
            // Consider the image already patched if it carries our own LKM, or if
            ...
            if already_patched {
                println!(
                    "- Image already carries init.real; reusing it (not overwriting the real init)"
                );
            }
            if !already_patched && cpio.exists("init") {
