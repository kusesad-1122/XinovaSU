...
enum Commands {
    ...
    /// Apply FolkPatch-style bootloader / verified-boot property hiding
    BlHide,
}
...
enum Kernel {
    ...
    /// Manage kernel uname spoofing
    UtsSpoof {
        #[command(subcommand)]
        command: UtsSpoofOp,
    },
    /// Manage per-app network isolation
    NetIsolate {
        #[command(subcommand)]
        command: NetIsolateOp,
    },
    /// Manage per-app path hiding
    PathHide {
        #[command(subcommand)]
        command: PathHideOp,
    },
    /// Manage per-app VPN-detection hiding
    VpnHide {
        #[command(subcommand)]
        command: VpnHideOp,
    },
...
enum UmountOp {
    ...
    Wipe,
    /// Persist the umount-service path list and apply it to the kernel
    Save {
        /// mount point paths
        paths: Vec<String>,
    },
    /// Apply the persisted umount-service list to the kernel
    Apply,
    /// Print the persisted umount-service list
    List,
}

#[derive(clap::Subcommand, Debug)]
enum UtsSpoofOp {
    /// Set spoofed uname release/version and persist
    Set {
        /// spoofed `uname -r` (release)
        release: String,
        /// spoofed `uname -v` (version)
        version: String,
    },
    /// Reset uname to the original values
    Reset,
    /// Re-apply the persisted spoof to the kernel
    Apply,
}

#[derive(clap::Subcommand, Debug)]
enum NetIsolateOp {
    /// Set the full isolation state (enabled + uid list) and persist
    Set {
...
#[derive(clap::Subcommand, Debug)]
enum VpnHideOp {
    /// Set the full vpn-hide state (enabled + uid list) and persist
    Set {
        /// enable vpn-detection hiding
        #[arg(long)]
        enabled: bool,
        /// target uids
        uids: Vec<u32>,
...
#[derive(clap::Subcommand, Debug)]
enum PathHideOp {
    /// Set the full path-hide state (enabled/filter-system/paths/uids) and persist
    Set {
        /// enable path hiding
        #[arg(long)]
        enabled: bool,
        /// also hide from system/root processes (reserved; limited in v1)
        #[arg(long)]
        filter_system: bool,
        /// hidden path (repeatable)
        #[arg(long = "path")]
        paths: Vec<String>,
        /// target app uid (repeatable)
        #[arg(long = "uid")]
        uids: Vec<u32>,
    },
    /// Re-apply the persisted state to the kernel
    ...
    let result = match cli.command {
        ...
        Commands::Kernel { command } => match command {
            ...
            Kernel::Umount { command } => match command {
                ...
                UmountOp::Save { paths } => crate::umount_service::save_list(&paths),
                UmountOp::Apply => {
                    crate::umount_service::apply_umount_list();
                    Ok(())
                }
                UmountOp::List => {
                    crate::umount_service::print_list();
                    Ok(())
                }
            },
            Kernel::UtsSpoof { command } => match command {
                UtsSpoofOp::Set { release, version } => {
                    crate::kernel_spoof::save(&release, &version)
                }
                UtsSpoofOp::Reset => crate::kernel_spoof::clear(),
                UtsSpoofOp::Apply => {
                    crate::kernel_spoof::apply_from_config();
                    Ok(())
                }
            },
            Kernel::NetIsolate { command } => match command {
                NetIsolateOp::Set { enabled, uids } => crate::net_isolate::save(enabled, &uids),
                NetIsolateOp::Apply => {
                    crate::net_isolate::apply_from_config();
                    Ok(())
                }
            },
            Kernel::PathHide { command } => match command {
                PathHideOp::Set {
                    enabled,
                    filter_system,
                    paths,
                    uids,
                } => crate::path_hide::save(enabled, filter_system, &paths, &uids),
                PathHideOp::Apply => {
                    crate::path_hide::apply_from_config();
                    Ok(())
                }
            },
        ...
        Commands::BlHide => crate::bl_hide::run_bl_hide(),
    };
